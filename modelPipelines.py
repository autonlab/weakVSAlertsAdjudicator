...
def processing_WS(X, y, order, lfs, X_drop = None):
    if X_drop is None:
        X_drop = X
    ...
    for train, test in tqdm(logo.split(X, y, groups=order['patientID'])):
        ...
        di1['positive_proba'] = RFModel.predict_proba(X_drop.iloc[test,:])[:,1]
...
def processing_FS(X, y, order, lfs, X_drop = None):
    if X_drop is None:
        X_drop = X
...
def processing_MajLab(X, y, order, lfs, X_drop = None):
    if X_drop is None:
        X_drop = X
...
def processing_ProbLab(X, y, order, lfs): #PROBABILITY LABELS don't have ablation settings
    logo = LeaveOneGroupOut()
