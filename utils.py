...
def formatProcessedData(compiledDataPath, alerts = pd.read_pickle("./preSP.pkl"), beginningCols = 11, alertType = 'spo2', output = 'df', telemetricDataPath = '/data/numerics/SpO₂T.SpO₂T:value', nontelemetricDataPath = '/data/numerics/SpO₂.SpO₂:value'):
    dataPiece = [f for f in os.listdir(compiledDataPath) if not f.startswith('.')]
    piece = pd.read_pickle(compiledDataPath + dataPiece[0])
    dfs = []
    for fn in dataPiece:
        dfs.append(pd.read_pickle(compiledDataPath + fn))
    result = pd.concat(dfs)
    result = result.sort_index()
    merged = pd.merge(alerts,result,on=alerts.index,how = 'outer')
    bigRep =(pd.concat([merged]*3)).sort_index()
    bigRep['windowID'] = [0,1,2]*merged.shape[0]
    bigRep = bigRep.reset_index(drop=True)
    dfLIST = []
    cols = bigRep.columns.values[beginningCols:]
    for i in range(len(bigRep.index)):
        window = bigRep.loc[i,'windowID']
        dfSmall = pd.DataFrame(columns = cols)
        for j in range(len(cols)):
            dfSmall.loc[i,cols[j]] = bigRep.loc[i,cols[j]][window]
        dfLIST.append(dfSmall)
    expand = pd.concat(dfLIST)
    squeegee = bigRep.iloc[:,:beginningCols]
    squeegee.rename(columns = {'key_0':'origin'}, inplace = True)
    bop = pd.merge(squeegee,expand,on=squeegee.index,how = 'outer')
    if (alertType == 'spo2'):
        needsDeleting = np.array(['med_art', 'std_art',
           'numdp_art', 'q1_art', 'q3_art', 'kurt_art', 'skew_art',
           'med_resp', 'std_resp', 'numdp_resp', 'q1_resp', 'q3_resp',
           'kurt_resp', 'skew_resp','artFFT', 'artINT', 'artNK1', 'artNK2']).astype(object)
        telemetricDF = {telemetricDataPath: 1, nontelemetricDataPath: 0}
        bop['telemetric'] = bop.series
        bop['telemetric'].replace(telemetricDF,inplace=True)
    elif (alertType == 'rr'):
        needsDeleting = np.array(['artFFT', 'artNK1', 'ecgiiiFFT', 'ecgiiiINT', 'ecgiiiNK1',
           'ecgiiiNK2', 'kurt_art', 'kurt_ecgiii', 'kurt_plethT',
           'kurt_spo2T', 'mean_art', 'mean_ecgiii', 'mean_plethT',
           'mean_spo2T', 'med_art', 'med_ecgiii', 'med_plethT', 'med_spo2T',
           'plethTFFT', 'plethTNK1', 'q1_art', 'q1_ecgiii',
           'q1_plethT', 'q1_spo2T', 'q3_art', 'q3_ecgiii', 'q3_plethT',
           'q3_spo2T', 'skew_art', 'skew_ecgiii', 'skew_plethT', 'skew_spo2T',
           'std_art', 'std_ecgiii', 'std_plethT', 'std_spo2T', 'tooEarly']).astype(object)
        one = []
        two = []
        for l in bop.plethNK1:
            one.append(l[0])
            two.append(l[1])
        bop['plethNK1_1'] = np.array(one)
        bop['plethNK1_2'] = np.array(two)
        bop.drop(columns=['plethNK1'], inplace = True)
    else:
        print('Given alert type not currently supported')
    more = bop.drop(columns = needsDeleting)
    part1 = more.iloc[:,:beginningCols + 1]
    part1.drop(columns = ['key_0'],inplace = True)
    part2 = more.iloc[:,beginningCols + 1:]
    part2_sorted = part2.sort_index(axis = 1)
    yea = []
    for col in part2_sorted.columns.values:
        if np.sum(part2_sorted.loc[:,col] < 0) > 0:
            yea.append(col)
    negs = np.array(yea)
    other = np.setdiff1d(part2_sorted.columns.values,negs)
    fillnegs = {k: 0 for k in negs}
    fillother = {k: -1 for k in other}
    a = part2.fillna(value = fillnegs)
    b = a.fillna(value = fillother)
    kl = negs.tolist() + other.tolist()
    c = b.loc[:,kl]
    pleasefinal = pd.merge(part1,c,on=c.index,how = 'outer')
    pleasefinal.filename[0].split('_')
    idcol = [pleasefinal.filename[i].split('_')[1] for i in range(len(pleasefinal))]
    di = {k:v for k,v in zip(pd.unique(idcol),range(len(np.unique(idcol))))}
    pleasefinal.rename(columns = {'key_0':'patientID'},inplace=True)
    pleasefinal['patientID'] = idcol
    pleasefinal['patientID'].replace(di,inplace = True)
    pid = pleasefinal.loc[:,'patientID']
    ind = pleasefinal.index
    order = pd.DataFrame()
    order['ind'] = ind
    order['patientID'] = pid
    X = pleasefinal.iloc[:,beginningCols + 1:]
    final = X
    final.insert(0,'patientID',pleasefinal.patientID.values)
    final.insert(0,'real_vs_art',pleasefinal.real_vs_art.values)
    if output == 'df':
        return final
    elif output == 'xy':
        return (X,y)
    else:
        print('That output is not currently supported')
        
def incompleteFeatures(df, threshold = .75):
    k = df.columns[12:].values
    d = dict.fromkeys(k)
    for key in k:
        NoNN = 0
        for num in df[key].values:
            NoNN = NoNN + (1 if math.isnan(num) else 0)
        d[key] = NoNN / df.shape[0]
    needsDeleting_ = k[np.array(list(d.values())) > threshold]
    return needsDeleting_

def positive_proba_binary(model, X_test):
    results = model.predict_proba(X_test)
    numClasses = len(model.classes_)
    if numClasses == 2:
        return results[:,1]
    elif numClasses == 1:
        val = 1 - model.classes_[0]
#         inter = np.zeros(len(results)) + val
        return np.abs(val - results.T[0])
    else:
        print('Some error occured in positive_proba_binary()')
        return results[:,1]